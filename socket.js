    ...
    io.on('connection', (socket) => {
        ...
        socket.on('send_msg', async (send_msg) => {
            try {
                ...
                if (saveMsg.constant_Id) {
                    const unReadMsg = await db.chats.count({
                        where: {
                            is_read: 0,
                            reciver_id: saveMsg.sender_id,
                            constant_Id: saveMsg.constant_Id
                        }
                    });

                    saveMsg.dataValues.unReadCnt = unReadMsg;
                }
        ...
        socket.on('read_msg', async (readChat) => {
            try {

                let msgRead = await db.chats.update(
                    { is_read: 1 },
                    {
                        where: {
                            sender_id: readChat.senderId,
                            reciver_id: readChat.reciverId,
                        },
                    }
                );

                if (!msgRead) {
                    console.log('Unable to mark as read in chat table');
                }

                socket.emit('readMsg', msgRead);
            } catch (error) {
                console.log('msg read socket error', error);
            }
        });
